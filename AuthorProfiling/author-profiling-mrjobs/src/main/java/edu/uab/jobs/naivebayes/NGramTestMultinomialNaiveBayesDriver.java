 ...
 */
public class NGramTestMultinomialNaiveBayesDriver extends Configured implements Tool {
    private static final Logger log = LoggerFactory.getLogger(NGramTestMultinomialNaiveBayesDriver.class);
    ...
    public int run(String[] args) throws Exception {
        if (args.length != 5) {
            System.err.printf(
                    "Usuage : %s [generic options ] <input> <output> <model> <dictionary> <model type [age| gender| agegender]> \n ",
        ...
        getConf().set(NGramTestMultinomialNaiveBayesDriver.MODEL_TYPE, args[4]);

        //add the dictionary and model path to cache
        DistributedCache.setCacheFiles(new URI[]{model.toUri(), new Path(args[3]).toUri()}, getConf());


        Job job = new Job(getConf());
        job.setJobName("Test multinomial naive bayes ");
        job.setJarByClass(NGramTestMultinomialNaiveBayesDriver.class);
        ...
        job.setMapperClass(NGramMultinomialNaiveBayesTestMapper.class);
    ...
    public static void main(String args[]) throws Exception {
        NGramTestMultinomialNaiveBayesDriver testRunner = new NGramTestMultinomialNaiveBayesDriver();
