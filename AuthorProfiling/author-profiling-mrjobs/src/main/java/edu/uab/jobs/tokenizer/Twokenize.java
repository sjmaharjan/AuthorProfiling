...
public class Twokenize {
    ...
    public static void main(String args[]) {
        String sentence = "I am goint .. to the Nepal??? What are you can't goin it's not ! !!! new to do :-):-):-)";
        Pattern Emoticons = Pattern.compile(emoticon);
        Pattern Punctuation = Pattern.compile(punctChars);
        System.out.println(Twokenize.tokenize(sentence));
        for (String token : Twokenize.tokenize(sentence)) {
            Matcher m1 = Emoticons.matcher(token);
            if (m1.find()) {
                System.out.println(token + " " + "Emoticons");
                continue;
            }
            Matcher m2 = Punctuation.matcher(token);
            Matcher m3 = Contractions.matcher(token);
            if (!m3.find() && m2.find()) {
